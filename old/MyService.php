...
$id_tipo_cuenta,$id_banco,$total_ingreso,$total_descuento) {
	...
	if ($connect) {
		   ...
		   id_banco='$id_banco',numero_cuenta='$total_ingreso',total_descuento='$total_descuento'");
           $result[] = array("msg" => "OK","id" => mysqli_insert_id($connect));
	}
	else
	{
...
function GetDiscountsIncomeEmployeeByid($id_descuento_ingreso_empleado) {
	...
	if ($connect) {
		 ...
		 monto_urgente, monto_original, monto_pendiente, estado, referencia, id_periodo, fecha_ultimo_pago, cantidad_pago, fecha_creacion, id_usuario 
		 ...
		 $qur=mysqli_query($connect,$sql);
			...
			if(mysqli_num_rows($qur))  {
               while($row = mysqli_fetch_assoc($qur)) {
					  ...
					  'referencia' => $referencia, 'id_periodo' => $id_periodo, 'fecha_ultimo_pago'=>$fecha_ultimo_pago, 'cantidad_pago'=>$cantidad_pago, 'fecha_creacion' => $fecha_creacion,'id_usuario'=> $id_usuario); 
				}
			}
			else
			{
					 $result[] = array('id_descuento_ingreso_empleado' => null, 'numero_cliente' => null, 
					  ...
					  'referencia' => null, 'id_periodo' => null, 'fecha_ultimo_pago'=>null, 'cantidad_pago'=>null, 'fecha_creacion' => null,'id_usuario'=> null); 
			}
...
function GetAllDiscountsIncomeEmployeeByidEmployee($id_empleado) {
	...
	if ($connect) {
		 ...
		 $sql="SELECT id_descuento_ingreso_empleado, numero_cliente, numero_cuenta, id_empleado, numero_empleado, 
		 id_descuento_ingreso, monto_mes, monto_periodo, afecta_diciembre, prioridad, tipo, frecuencia, 
		 monto_urgente, monto_original, monto_pendiente, estado, referencia, fecha_ultimo_pago, cantidad_pago, fecha_creacion, id_usuario 
		 FROM descuentos_ingresos_empleados
		 WHERE id_empleado='$id_empleado'";
		 $qur=mysqli_query($connect,$sql);
			...
			if(mysqli_num_rows($qur))  {
               while($row = mysqli_fetch_assoc($qur)) {
					  ...
					  'estado' => $estado,'referencia' => $referencia,'fecha_ultimo_pago'=>$fecha_ultimo_pago, 'cantidad_pago'=>$cantidad_pago,'fecha_creacion' => $fecha_creacion,'id_usuario'=> $id_usuario); 
				}
			}
